createValidationPackage <- function(modelFolder, 
  ...
  transportCohort(packageName = "SimpleHospitalizationInFluPatients",
...
transportPlpModels <- function(analysesDir,
  ...
  if(length(cohortCovs)>0 & !is.null(cohortVariableSetting)){
    ...
    pathToCustom <- system.file("settings", cohortVariableSetting, package = "SimpleHospitalizationInFluPatients")
...
transportCohort <- function(packageName = "SimpleHospitalizationInFluPatients",
                            outputDir = "./inst"){
